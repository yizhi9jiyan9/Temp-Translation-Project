scoreboard players set @s ItemStmr 400
...
#summon 
summon zombified_piglin ~ ~ ~ {Health:10f,Tags:["zombie_piglin"],HandItems:[{id:"minecraft:golden_sword",Count:1b},{}],Attributes:[{Name:"generic.max_health",Base:10},{Name:"generic.movement_speed",Base:.4}],AngryAt:[I;1615399313,-900382376,-1387954485,1805930413]}
data modify storage items:piglin UUID set from entity @a[tag=target,limit=1] UUID
data modify entity @e[type=zombified_piglin,tag=zombie_piglin,tag=!setup,limit=1] AngryAt set from storage items:piglin UUID
data remove storage items:piglin UUID
scoreboard players operation @e[type=zombified_piglin,tag=zombie_piglin,tag=!setup,limit=1] HitID = @s HitID
tag @e[type=zombified_piglin,tag=zombie_piglin,tag=!setup,limit=1] add setup
