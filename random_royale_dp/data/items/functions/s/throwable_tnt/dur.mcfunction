...
#snowball is yesir 
execute at @s as @e[type=snowball,distance=..3,sort=nearest,tag=!setup,limit=1] run tag @s add tnt
execute at @s run scoreboard players operation @e[type=snowball,distance=..3,sort=nearest,tag=!setup,limit=1] OuchID = @s HitID
execute as @e[type=snowball,tag=!setup,tag=tnt] run function items:s/throwable_tnt/glob_setup