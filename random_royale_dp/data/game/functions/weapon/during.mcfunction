...
execute unless entity @a[tag=v,scores={Player=1..2}] run bossbar set game name [{"text":"Current weapon leader: ","color":"gold"},{"text":"N/A","color":"aqua"}]
execute if score #store FFAScores matches 1 run bossbar set game name [{"text":"Current weapon leader: ","color":"gold"},{"selector":"@a[tag=v]","color":"aqua"}]
execute if score #store FFAScores matches 2.. run bossbar set game name [{"text":"Current weapon leader: ","color":"gold"},{"text":"contested","color":"aqua"}]
tag @a[scores={Player=1..2}] remove v

#setting new item is done in weapon/die

#tell what the next weapon is 
execute as @a[scores={Player=1}] run function game:weapon/actionbar
